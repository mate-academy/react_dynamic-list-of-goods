import React, { useState } from 'react';
import './App.scss';
import { GoodsList } from './GoodsList';

import { getAll, get5First, getRedGoods } from './api/goods';
// or
// import * as goodsAPI from './api/goods';

import { Good } from './types/Good';

export const App: React.FC = () => {
  const [showGoods, setShowGoods] = useState<Good[]>([]);

  async function showAll() {
    const showAllGoods = await getAll();

    setShowGoods(showAllGoods);
  }

  async function showFive() {
    const fiveGoods = await get5First();

    setShowGoods(fiveGoods);
  }

  async function showRed() {
    const redGoods = await getRedGoods();

    setShowGoods(redGoods);
  }

  return (
    <div className="App">
      <h1>Dynamic list of Goods</h1>

      <button type="button" data-cy="all-button" onClick={showAll}>
        Load all goods
      </button>

      <button type="button" data-cy="first-five-button" onClick={showFive}>
        Load 5 first goods
      </button>

      <button type="button" data-cy="red-button" onClick={showRed}>
        Load red goods
      </button>

      <GoodsList goods={showGoods} />
    </div>
  );
};
